...
class GitHubAPIClient:
    ...
    def get_paginated_results(self, url: str, params: Optional[Dict[str, Any]] = None, limit: Optional[int] = None) -> List[Any]:
        ...
        while True:
            ...
            if not isinstance(page_results, list):
                logger.warning(f"Expected list but got {type(page_results)} for page {page} of {url}. Stopping pagination.")
                break
...
class DataCollector:
    ...
    def get_commits(self, owner: str, repo: str, since_date: Optional[str] = None, until_date: Optional[str] = None, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def get_issues(self, owner: str, repo: str, state: str = 'all', since_date: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
...
class MetricsCalculator:
    ...
    def calculate_release_frequency_score(self, releases: List[Dict[str, Any]], months_period: int) -> Tuple[float, int]:
    ...
    def calculate_avg_issue_open_time_score(self, open_issues: List[Dict[str, Any]]) -> float:
        ...
        for issue in open_issues:
            if 'pull_request' in issue: continue # Skip PRs listed as issues
            try:
...
class ComponentCalculator:
    ...
    def calculate_activity(self, owner: str, repo: str) -> ComponentData:
        ...
        commits = self.data_collector.get_commits(
            owner, repo, since_date=self.six_months_ago_str, limit=500 # Limit to recent 500
        )
        ...
        recent_activity_score, days_since_last_commit = self.metrics.calculate_recent_activity_score(latest_commit_date_str)
        ...
        activity_score = (0.4 * commit_frequency_score +
                          0.4 * recent_activity_score +
                          0.2 * release_frequency_score)

        logger.debug(f"Activity: Commits(6m)={num_commits_6m}, Releases(6m)={num_releases_6m}, DaysSinceCommit={days_since_last_commit}")
        logger.debug(f"Activity Scores: CF={commit_frequency_score:.2f}, RA={recent_activity_score:.2f}, RF={release_frequency_score:.2f} -> Overall: {activity_score:.2f}")
    ...
    def calculate_maintenance(self, owner: str, repo: str, repo_data: Dict[str, Any]) -> ComponentData:
        ...
        update_cadence_score, days_since_last_update = self.metrics.calculate_update_cadence_score(repo_updated_at) if repo_updated_at else (0.0, 9999)

        # Calculate overall maintenance score (adjust weights as needed)
        maintenance_score = (0.35 * avg_issue_open_time_score +
                             0.35 * pr_merge_rate_score +
                             0.30 * update_cadence_score)

        logger.debug(f"Maintenance: OpenIssues={num_open_issues}, ClosedPRsChecked={num_closed_prs}, DaysSinceUpdate={days_since_last_update}")
        logger.debug(f"Maintenance Scores: IssueTime={avg_issue_open_time_score:.2f}, PRMerge={pr_merge_rate_score:.2f}, UpdateCad={update_cadence_score:.2f} -> Overall: {maintenance_score:.2f}")
    ...
    def calculate_documentation(self, owner: str, repo: str, repo_data: Dict[str, Any]) -> ComponentData:
        ...
        contribution_guidelines_score = self.metrics.calculate_contribution_guidelines_score(has_contributing, has_code_of_conduct)

        # Check for examples directory
        has_examples = (self.data_collector.check_dir_exists(owner, repo, 'examples') or
                        self.data_collector.check_dir_exists(owner, repo, 'samples') or
                        self.data_collector.check_dir_exists(owner, repo, 'docs/examples')) # Check common paths
        ...
        documentation_score = (0.4 * readme_score +
                               0.3 * contribution_guidelines_score +
                               0.15 * examples_score +
                               0.15 * wiki_score)

        logger.debug(f"Documentation Checks: README={has_readme}, CONTRIB={has_contributing}, CoC={has_code_of_conduct}, Examples={has_examples}, Wiki={has_wiki}")
        logger.debug(f"Documentation Scores: Readme={readme_score:.2f}, Guidelines={contribution_guidelines_score:.2f}, Examples={examples_score:.2f}, Wiki={wiki_score:.2f} -> Overall: {documentation_score:.2f}")
    ...
    def calculate_community(self, owner: str, repo: str, repo_data: Dict[str, Any]) -> ComponentData:
        ...
        closed_issues_raw = self.data_collector.get_issues(owner, repo, state='closed', limit=200) # Get recent closed issues
        ...
        has_contributing = getattr(self, '_has_contributing', self.data_collector.check_file_exists(owner, repo, 'CONTRIBUTING.md'))
        has_code_of_conduct = getattr(self, '_has_code_of_conduct', self.data_collector.check_file_exists(owner, repo, 'CODE_OF_CONDUCT.md'))
        ...
        community_score = (0.4 * contributor_growth_score +
                           0.3 * issue_activity_score +
                           0.3 * community_health_score)

        logger.debug(f"Community: Contribs={contributor_count}, OpenIssues={open_issues_count}, TotalIssuesEst={total_issues_approx}")
        logger.debug(f"Community Scores: Growth={contributor_growth_score:.2f}, IssueActivity={issue_activity_score:.2f}, HealthFiles={community_health_score:.2f} -> Overall: {community_score:.2f}")
...
class RepoRater:
    ...
    def rate_repository(self, owner: str, repo: str) -> Optional[ResultData]:
        ...
        final_score = sum(comp.score * comp.weight for comp in [
            stars_comp, activity_comp, maintenance_comp, documentation_comp, community_comp
        ])
    ...
    def _interpret_score(score: float) -> str:
        ...
        elif score >= 0.65: return "Very Good"
...
class ResultsFormatter:
    ...
    def format_text(results: Optional[ResultData]) -> str:
        ...
        for name, comp_data in results.components.items():
            output.append(f"[{name.capitalize()}]".ljust(pad) + f"Score: {comp_data.score:.3f} (Weight: {comp_data.weight * 100:.0f}%)")
            ...
            if isinstance(data, dict) and 'count' in data: # Stars
                 ...
                 output.append(f"{'  Commit Freq Score:'.ljust(pad)} {data.commit_frequency_score:.3f} ({data.num_commits_6months} commits/6mo)")
                 output.append(f"{'  Recent Activity Score:'.ljust(pad)} {data.recent_activity_score:.3f} ({data.days_since_last_commit} days ago)")
                 output.append(f"{'  Release Freq Score:'.ljust(pad)} {data.release_frequency_score:.3f} ({data.num_releases_6months} releases/6mo)")
            elif isinstance(data, MaintenanceData):
                 output.append(f"{'  Avg Issue Open Score:'.ljust(pad)} {data.avg_issue_open_time_score:.3f} ({data.num_open_issues} open)")
                 output.append(f"{'  PR Merge Rate Score:'.ljust(pad)} {data.pr_merge_rate_score:.3f} (from {data.num_closed_prs_recent} recent closed PRs)")
                 output.append(f"{'  Update Cadence Score:'.ljust(pad)} {data.update_cadence_score:.3f} ({data.days_since_last_update} days ago)")
            elif isinstance(data, DocumentationData):
                 output.append(f"{'  README Score:'.ljust(pad)} {data.readme_score:.3f} (Exists: {data.has_readme}, Size: {data.readme_size})")
                 output.append(f"{'  Guidelines Score:'.ljust(pad)} {data.contribution_guidelines_score:.3f} (CONTRIB: {data.has_contributing}, CoC: {data.has_code_of_conduct})")
                 output.append(f"{'  Examples Score:'.ljust(pad)} {data.examples_score:.3f} (Exists: {data.has_examples})")
                 output.append(f"{'  Wiki Score:'.ljust(pad)} {data.wiki_score:.3f} (Enabled: {data.has_wiki})")
            elif isinstance(data, CommunityData):
                 output.append(f"{'  Contributor Growth Score:'.ljust(pad)} {data.contributor_growth_score:.3f} ({data.contributor_count} contributors)")
                 output.append(f"{'  Issue Activity Score:'.ljust(pad)} {data.issue_activity_score:.3f} ({data.open_issues} open / {data.total_issues_approx} total est.)")
                 output.append(f"{'  Community Health Score:'.ljust(pad)} {data.community_health_score:.3f} (Based on CONTRIB/CoC files)")
...
def main(*, token: str | None, owner: str, repo: str) -> None:
...
if __name__ == "__main__":
    ...
    main(token=token, owner=args.owner, repo=args.repo)